...
import { getOverviewofTrades } from "@/api/trades-api";

const TradesOverviewCards = () => {
  ...
  useEffect(() => {
    const fetchOverviewDetails = async () => {
      try {
        const response = await getOverviewofTrades(navigate);
...
export default TradesOverviewCards;
