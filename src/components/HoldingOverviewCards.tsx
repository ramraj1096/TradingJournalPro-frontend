...
import { getOverview } from "@/api/holding-api";
import { useNavigate } from "react-router-dom";
import { toast } from "sonner";

const HoldingOverviewCards = () => {
  ...
  useEffect(() => {
    const fetchOverviewDetails = async () => {
      try {
        const response = await getOverview(navigate);
        console.log(response);
        if (response.success) {
          setTotalInvestedValue(response.totalInvestment);
  ...
  return (
    <div className="flex flex-wrap justify-center gap-4 p-4 md:flex-nowrap">
      ...
      <Card className="w-full max-w-xs md:max-w-sm">
        <CardHeader>
          <h1 className="text-lg font-bold text-gray-700 dark:text-slate-300">Total Invested Value</h1>
        </CardHeader>
        <CardContent>
      ...
      <Card className="w-full max-w-xs md:max-w-sm">
        <CardHeader>
          <h1 className="text-lg font-bold text-gray-700 dark:text-slate-300">Current Investment Value</h1>
        </CardHeader>
        <CardContent>
...
export default HoldingOverviewCards;
